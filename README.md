...
- **下端のパネルは1つの枠**: 画面内キーボード・マウスパネル・OSキーボード用の入力欄は、左右の端も背景も高さも同じ「トレイ」に収める。切り替わるのは中身だけで、**映像の位置も大きさも動かない**(以前はキーボード250px・マウス140pxと高さが違ううえ、キーボードのときだけ拡大表示になっていたので、切り替えるたびに映像が伸び縮みして見ている場所を見失った)。トレイが出ているあいだ映像は残った領域を余白なく埋め、切り取られて見えない側へカーソルが寄ると**映像の方がずれて追う**
