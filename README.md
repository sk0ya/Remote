...
- **認証**: PCまでのP2P経路が成立したことを確認してから、顔認証/指紋でパスキー署名。認証完了までは映像・操作をロックする。**スマホ側に秘密情報を一切保存しない**ので、同じ端末なら別のブラウザからでも、パスキーが同期していれば別の端末からでも繋がる
...
- **文字入力**: 変換をどちらでやるか選べる。既定の**PC変換**は打鍵をスキャンコードのまま送るので、PC側のIMEが受け取り、**予測変換・変換学習・単語登録がPCのまま**使える(候補ウィンドウは映像に映る)。**スマホ変換**へ切り替えるとスマホのIMEで確定した文字列を送る
...
- offer/answerを先に適用してP2P疎通を確認し、その後 `SHA-256(nonce ‖ offerSDP ‖ answerSDP)` をチャレンジとしてパスキーで署名。認証完了までは画面キャプチャを開始せず、DataChannelの入力も破棄する。nonceはホストが接続ごとに発番するのでリプレイ不可、SDPを混ぜてあるので中継サーバーが書き換えれば検証で落ちる(旧HMAC方式の役割を兼ねる)
