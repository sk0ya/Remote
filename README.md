...
- **キーボード**: OSのソフトキーボードは使わず、押したキーをスキャンコードのままPCへ送る。文字面(ABC)と数字・記号面(123)を切り替える方式で、修飾キー・矢印・Esc/Tab/Del・半角/全角 は面をまたいで同じ位置に出る「操作段」に置く。修飾キーは押しっぱなしになり次の1打で外れるので、Ctrl+C や Alt+Tab を1打ずつ打てる
- **日本語入力**: 対応しない(かなを直接入力する手段は持たない)。ローマ字はPC側のIMEが受け取るので、PCのIMEを使えば変換はできる。IMEの入切は操作段の「半/全」キー(JIS配列の半角/全角と同じスキャンコード)
