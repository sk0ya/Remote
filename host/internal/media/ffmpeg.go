...
var pipelines = [][]string{
	{"-filter_complex", grabFilter(""), "-c:v", "h264_nvenc", "-preset", "p1", "-tune", "ull", "-zerolatency", "1"},
	{"-filter_complex", grabFilter(""), "-c:v", "h264_amf", "-usage", "ultralowlatency"},
	{"-filter_complex", grabFilter(",hwdownload,format=bgra"), "-c:v", "h264_qsv", "-preset", "veryfast"},
	{"-filter_complex", grabFilter(",hwdownload,format=bgra"), "-c:v", "libx264", "-preset", "ultrafast", "-tune", "zerolatency", "-profile:v", "baseline", "-pix_fmt", "yuv420p"},
	// Desktop Duplication が使えない環境向けの最終フォールバック
	{"-f", "gdigrab", "-framerate", fmt.Sprint(frameRate), "-i", "desktop", "-c:v", "libx264", "-preset", "ultrafast", "-tune", "zerolatency", "-profile:v", "baseline", "-pix_fmt", "yuv420p"},
}

func grabFilter(post string) string {
	return fmt.Sprintf("ddagrab=framerate=%d%s", frameRate, post)
}

func buildArgs(pipeline []string) []string {
	args := []string{"-hide_banner", "-loglevel", "error"}
	args = append(args, pipeline...)
	args = append(args,
		"-b:v", bitrate, "-maxrate", bitrate, "-bufsize", "8M",
		"-g", fmt.Sprint(frameRate), // GOP 1秒: IDR強制ができない代わりに短周期
		"-bf", "0",
		"-f", "h264", "-",
	)
	return args
...
func Capture(ctx context.Context, ch chan<- Sample) error {
	for i, p := range pipelines {
		err := runPipeline(ctx, p, ch)
		if ctx.Err() != nil {
			return ctx.Err()
		}
...
func runPipeline(ctx context.Context, pipeline []string, ch chan<- Sample) error {
	cmd := exec.CommandContext(ctx, "ffmpeg", buildArgs(pipeline)...)
	...
	err = readAccessUnits(bufio.NewReaderSize(stdout, 1<<20), ch, firstFrame)
...
func readAccessUnits(r *bufio.Reader, ch chan<- Sample, firstFrame chan struct{}) error {
	var buf []byte      // 未処理バイト
	var au []byte       // 組み立て中のアクセスユニット
	auHasVCL := false   // 現在のAUにスライスNALが含まれるか
	first := true
	frameDur := time.Second / frameRate
